...
table1::table1(
  ~ sex + pre_score + post_score + age + war + sexual + physical + 
...
t_test
```
The results from the pre-test (M = `r filter(vet, veteran == "Veteran")[, "pre_score"] %>% mean() %>% round(1)`, SD = `r filter(vet, veteran == "Veteran")[, "pre_score"] %>% mean() %>% sd(1)`) and post-test (M = `r filter(vet, veteran == "Veteran")[, "post_score"] %>% mean() %>% round(1)`, SD = `r filter(vet, veteran == "Veteran")[, "post_score"] %>% sd() %>% round(1)`) CAPS interview indicate that the treatment significantly reduced symptoms of PTSD in veterans with war-related trauma, t(`r t_test[["parameter"]][["df"]]`) = `r t_test[["statistic"]][["t"]] %>% round(1)`, p `r ifelse(t_test[["p.value"]] < 0.001, "< 0.001", cat("=", t_test[["p.value"]] %>% round(2)))`.
